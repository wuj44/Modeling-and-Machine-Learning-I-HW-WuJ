...
From the plot above, we can see the distribution of average test error with a series of k values and the range of standard deviation of the five estimates from each fold. As k goes up, the CV-estimated test error first goes down and then goes up, while the standard deviation goes down. It means when k is big enough, CV-estimated test error will not deviate much. According to the plot with training error, aic, bic, validation error and the plot above, we can choose 6 for k as when k=6, we can have the model with small training error, aic, bic, validation error, the least CV-estimated test error and the standard deviation is acceptable.
