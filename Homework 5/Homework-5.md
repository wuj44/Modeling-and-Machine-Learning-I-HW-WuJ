...
smaller. Based on the minimal test error criteria, we can find that when
the tuning parameter is 4, we can get the best model with minimal test
error. Based “one standard error” rule, we should find the simplest
model within one standard error of the test error of the best model. For
a kNN model, a bigger k value makes the model simpler. Therefore, 7
seems a good candidate for k.
